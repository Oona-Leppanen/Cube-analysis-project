...
#######################################
# Credits from this cell's code to:
...
for x in range(0, len(open_file)):
    ...
    deck_names.append(file_name)
    deck_dates.append(df.iat[len(df)-4, 0])
    deck_results.append(df.iat[len(df)-3, 0])

decks = pd.DataFrame(list(zip(deck_names, deck_dates, deck_results)), columns = ['Name', 'Date', 'Draft_result'])


# Creating new columns and filling them.
# This block can be run just once!!!
# If runned 2 or more times Same_date_different_draft column starts to get wrong values.

# Creates columns for victories, losses and ties of decks and a column for marking how many drafts has been drafted. Also Score
# column is created. Checks if there's been multiple drafts in the same date and adds the information about the second draft to
# Same_date_different_draft column if true. Fills all other created columns.
# Scores are counted as follows: 3 points for 1 victory, 1.5 point for 1 draw and 0 point for loss.

decks['Same_date_different_draft'] = 'Draft 1'
decks['Victory'] = 0
decks['Loss'] = 0
decks['Draw'] = 0
decks['Score'] = 0

for i in range(0, len(decks['Draft_result'])):
    score = 0
...
decks['Draft_result'] = decks.Draft_result.str.split(',', expand = True)[0] # Saves only the results to Draft_result column.
print(decks['Score'], '\n', decks['Same_date_different_draft'].values)

# Adds colors and types of decks to deck dataframe.

decks[['White', 'Blue', 'Black', 'Red', 'Green']] = 1
decks[['Aggro', 'Midrange', 'Control', 'Combo', 'Ramp']] = 1
s = []
colors = []
types = []
#print(decks)
...
for i in range(0, len(decks['Date'])):
...
decks['In_card_data'] = 0
    
print(decks)
print(decks.values)

decks.to_csv('original_deck_data.csv', sep = ';', index = False) # Saves the deck dataframe to a csv file.
...
# Initilizing some variables and dataframes.

open_file = glob.glob('Drafts_csv/*')
cards = pd.DataFrame(columns = ['Card_name'])
cards['Frequency_of_cards'] = 0
cards['Main_deck_count'] = 0
cards['Sideboard_count'] = 0
cards['Main_deck_rate'] = 0
cards['Sideboard_rate'] = 0
cards['Points_rate'] = 0

lands =  {'Type': ['Plains', 'Island', 'Swamp', 'Mountain', 'Forest'], 'Frequency_of_decks': 0, 'Number_of_lands': 0}
basic_lands = pd.DataFrame(lands)
print(basic_lands)

# Fills the cards dataframe with names, amounts, main deck count and sideboard count of cards.
# Also fills the basic_lands dataframe.

is_in_cards = 0

for x in range(0, len(open_file)):
    deck = pd.read_csv(open_file[x], delimiter = ';')
    ...
    for i in range(0, len(deck)):
        ...
        if (test_card == 'Plains') | (test_card == 'Island') | (test_card == 'Swamp') | (test_card == 'Mountain') | (test_card =='Forest'):
            ...
            for j in range(0, len(cards)):
                ...
                if cards.iloc[j]['Card_name'] == test_card:
                    cards.loc[j, 'Frequency_of_cards'] += 1
                    is_in_cards = 1
                    if deck.iloc[i]['Is_in_sideboard'] == 0:
                        cards.loc[j, 'Main_deck_count'] += 1
                    else:
                        cards.loc[j, 'Sideboard_count'] += 1
        ...
        if is_in_cards == 0:
            ...
            cards = pd.concat([cards, new_row], ignore_index = True, axis = 0)
        is_in_cards = 0
        

# Fills cards dataframe's Main_deck_rate and Sideboard_rate columns showing just 5 decimals.
for i in range(0, len(cards)):
    cards.loc[i, 'Main_deck_rate'] = format((cards.iloc[i]['Main_deck_count'] / cards.iloc[i]['Frequency_of_cards']), '.5f')
    cards.loc[i, 'Sideboard_rate'] = format((cards.iloc[i]['Sideboard_count'] / cards.iloc[i]['Frequency_of_cards']), '.5f')

deck_data = pd.read_csv('original_deck_data.csv', delimiter = ';')
cards_and_their_scores = []

# Fills cards_and_their_scores list with names of the cards and their scores from each deck.
for j in range(0, len(deck_data)):
    ...
    for x in range(0, len(open_file)):
        ...
        if deck_data.iloc[j]['Name'] == file_name:
            for i in range(0, len(deck)):
...
for x in range(0, len(cards)):
    for i in range(0, len(cards_and_their_scores), 2):
        ...
        if cards.iloc[x]['Card_name'] == cards_and_their_scores[i]:
            score_sum += cards_and_their_scores[i+1]
            score_amount += 1
            
    # Must be checked because basic lands are included cards_and_their_scores varible
    if score_amount != 0:
        cards.loc[x, 'Points_rate'] = score_sum / (score_amount*9)
        
    # If the card has not been in any deck then it's point rate is 0.
    elif score_amount == 0:
        cards.loc[x, 'Points_rate'] = 0.0

    score_amount = 0
    score_sum = 0
    print(cards.iloc[x]['Card_name'], ' ', cards.iloc[x]['Points_rate'])

# Creates a csv files from the basic_lands and cards dataframes.

print(cards['Points_rate'].values)
print(basic_lands)

deck_data.to_csv('original_deck_data.csv', sep = ';', index = False)
basic_lands.to_csv('original_basic_lands.csv', sep = ';', index = False) # Saves the basic_land dataframe to a csv file.
cards.to_csv('original_card_data.csv', sep = ';', index = False)
...
# Initializing dataframes and adding more columns to one of them.

card_data = pd.read_csv('original_card_data.csv', delimiter = ';') # Change to card_data.csv!
cube482022 = pd.read_csv('Finevintage482022.csv', delimiter = ';') # Downloaded data. Named as 'Finevintage482022'.
cards_fill_features = pd.DataFrame(columns = ['Card_name', 'Color', 'Mana_value', 'Type'])

#card_data[['White', 'Blue', 'Black', 'Red', 'Green']] = 1
card_data[['Color']] = ''
card_data['Mana_value'] = 0
card_data['Type'] = ''

print(cube482022)

# Fills created columns with values from cube482022 dataframe. If values are not found to a specific card name add that card 
# name to a cards_fill_features dataframe. Also deletes substrings from Type column if there are some unnecessary text along
# the main type (legendary/snow).

for i in range(len(card_data)):
    not_found = 0
    
    for j in range(len(cube482022)):
        if card_data.iloc[i]['Card_name'] == cube482022.iloc[j]['Name']:
            card_data.loc[i, 'Color'] = cube482022.iloc[j]['Color']
            card_data.loc[i, 'Mana_value'] = cube482022.iloc[j]['CMC']
            card_data.loc[i, 'Type'] = cube482022.iloc[j]['Type']
            not_found = 1
            
    if 'Legendary' in card_data.iloc[i]['Type']:
        card_data.loc[i, 'Type'] = card_data.iloc[i].Type[10:]
        
    if 'Snow' in card_data.iloc[i]['Type']:
        card_data.loc[i, 'Type'] = card_data.iloc[i].Type[5:]
    
    if not_found == 0:
        cards_fill_features.loc[i, 'Card_name'] = card_data.iloc[i]['Card_name']
        
print(cards_fill_features['Card_name'].values)
print(card_data)

# Saves changes of dataframes to csv files.

cards_fill_features.to_csv('cards_ff.csv', sep = ';', index = False)
card_data.to_csv('original_card_data.csv', sep = ';', index = False)
